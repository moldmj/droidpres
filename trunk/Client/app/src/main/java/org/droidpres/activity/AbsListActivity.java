...
public abstract class AbsListActivity extends ListActivity {
	...
	protected AbsListActivity(int contentId, String table_name,
			String[] fields, String order_by, String pricelist_id) {
		this.contentId = contentId;
		mQueryHelper = new QueryHelper(table_name, fields, order_by);
		if(table_name=="product")
		{
			mQueryHelper.appendJoin("Products_Prices", "_id", QueryHelper.JOIN_INNER, new String[]{"price"});
            mQueryHelper.appendJoin("Products_available", "_id", QueryHelper.JOIN_INNER, new String[]{"available"});
			//mQueryHelper.appendFilter("pricelist_id", QueryHelper.FILTER_AND, "pricelist_id = 2");//+Const.EXTRA_CLIENT_PRICE_ID);//+pricelist_id);			
		}
        if(table_name=="Characteristic")
        {
            mQueryHelper.appendJoin("Products_available", "_id", QueryHelper.JOIN_INNER, new String[]{"available"});
            mQueryHelper.appendJoin("Products_Prices", "_id", QueryHelper.JOIN_INNER, new String[]{"price"});
        }
