...
public class TransferActivity extends Activity implements OnClickListener{
    ...
    private static final int SERVER_RESERVE 		= 2;
	...
	private Button sBtImport,sBtImpExp;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		sBtImpExp = (Button) findViewById(R.id.btImpExp);
        ...
        sBtImpExp.setOnClickListener(this);
	...
	public void onClick(View v) {
		sTvTrLog.setText("");
        ExchangeTimeout = SetupRootActivity.getExchangeTimeout(this);
		...
		case SERVER_MOBILE:
			...
			//else if (v.getId() == R.id.btExport)
			//	new ExportToRCD().execute(sURL);
            else if  (v.getId() == R.id.btImpExp)
                new ExportToRCDAndImportProductAvailable().execute(sURL);
		...
		case SERVER_WIFI:
            ...
            break;
        case SERVER_RESERVE:
                sURL = SetupRootActivity.getReserveServer(this);
                if (sURL.trim().length() == 0) {
                    log(getString(R.string.err_ServerPort), "No setup url transfer server.");
                    return;
                }
		...
		protected Void doInBackground(String... params) {
			try {
				...
				sClientXMLRPC = new XMLRPCClient(params[0], SetupRootActivity.getHttpLogin(
						TransferActivity.this),	SetupRootActivity.getHttpPasswd(TransferActivity.this),ExchangeTimeout);
				...
				if (cur_doc.moveToFirst()) {
					do {
						...
						dict.put("isdiscount", QueryHelper.fieldByNameInt(cur_doc, "isdiscount"));
                        dict.put("warehouse_id", QueryHelper.fieldByNameInt(cur_doc, "warehouse_id"));
						...
						if (cur_docdet.moveToFirst()) {
							do {
                                ...
                                mapDocDet.put("characteristic_id", QueryHelper.fieldByNameInt(cur_docdet, "characteristic_id"));
	...
	private class ImportFromRCD extends AsyncTask<String, String, Void> {
		...
		protected Void doInBackground(String... params) {
			try {
                ...
                publishProgress(getString(R.string.msg_GetRefCharacteristic), null);
                getReferenceFromRCD(sDataBase, DBDroidPres.TABLE_CHARACTERISTIC, "GetRefCharacteristic");
                ...
                publishProgress(getString(R.string.msg_GetRefPrices), null);
                getReferenceFromRCD(sDataBase, DBDroidPres.TABLE_PRICELISTS, "GetRefPrices");
                publishProgress(getString(R.string.msg_GetRefProductPrices), null);
                getReferenceFromRCD(sDataBase, DBDroidPres.TABLE_PRODUCTS_PRICES, "GetRefProductPrices");
                publishProgress(getString(R.string.msg_GetRefWarehouse), null);
                getReferenceFromRCD(sDataBase, DBDroidPres.TABLE_WAREHOUSE, "GetRefWarehouse");
                publishProgress(getString(R.string.msg_GetRefProducts_available), null);
                getReferenceFromRCD(sDataBase, DBDroidPres.TABLE_PRODUCTS_AVAILIBLE, "GetRefProductsAvailable");
    ...
    //Ещё один костыль из-за нехватки времени...
    private class ExportToRCDAndImportProductAvailable extends AsyncTask<String, String, Void> {
        @Override
        protected Void doInBackground(String... params) {
            try {
                //Начали отправку документов
                publishProgress(getString(R.string.msg_StartExport), null);
                ...
                publishProgress(getString(R.string.msg_StopExport) , null);
                publishProgress(getString(R.string.msg_SendDocCount, send_doc_coun) , null);
                //Закончили отправку документов

                //Начали обновление остатков
                publishProgress(getString(R.string.msg_StartImport, params[0]), null);
                sClientXMLRPC = new XMLRPCClient(params[0], SetupRootActivity.getHttpLogin(
                        TransferActivity.this), SetupRootActivity.getHttpPasswd(TransferActivity.this),ExchangeTimeout);
                publishProgress(getString(R.string.msg_GetRefProducts_available), null);
                getReferenceFromRCD(sDataBase, DBDroidPres.TABLE_PRODUCTS_AVAILIBLE, "GetRefProductsAvailable");
                publishProgress(getString(R.string.msg_StopImportProductAvailable), null);
                //Закончили обновление остатков
