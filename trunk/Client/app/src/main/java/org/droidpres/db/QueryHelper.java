...
public class QueryHelper {
	...
	private String getTables() {
		...
		for (String table_join: map_join.keySet()) { 
			...
			{
                if (table_join=="Products_Prices")
                {
                    sb.append(join_str + table_join + " ON (" +
                            table_name + "." + map_join.get(table_join).join_field +  " = " +table_join + ".product_id) ");
                }
                else
                {
			...
			else if(table_name=="Characteristic")
            {
                if (table_join=="Products_available")
                {
                    sb.append(table_name+join_str + table_join + " ON (" +
                            table_name + "._id  ="+ table_join + ".characteristic_id) ");
                }
                else
                {
                    sb.append(join_str + table_join + " ON (" +
                            table_name + ".product_id  ="+ table_join + ".product_id) ");
                }
            }
            else
			{
	...
	public static byte[] fieldByNameBute(Cursor cursor, String field_name) {
		return cursor.getBlob(cursor.getColumnIndex(field_name));
	}
